...
def preparar_datos(df_train, df_test, feature_cols, target):
    """Preparar y validar datos para el entrenamiento"""
...
def main():
    ...
    for target in target_cols:
        ...
        X_train, y_train, X_test, y_test = preparar_datos(
            df_train, df_test, feature_cols, target
        )
        ...
        model = LGBMRegressor(**mejores_params[target], random_state=42)
        ...
        plot_predicciones(
            y_test, y_pred,
            f'Predicciones LightGBM para t+{horizonte} (Test)',
            f'graficos/lightgbm/lightgbm_predicciones_t{horizonte}.png'
        )

        # Graficar importancia de características
        importancia = plot_importancia_caracteristicas(
            model, feature_cols, horizonte,
    ...
    resultados_df.to_csv('resultados/lightgbm/resultados_lightgbm.csv')
    ...
    print("- resultados/lightgbm/resultados_lightgbm.csv")
